...
if (!urlPattern.test(url)) { log(`Error: Invalid URL pattern: ${url}`); return; }
...
} catch (error) { log(`Failed to open browser: ${error.message}`); log('Please open the URL manually'); }
}
...
if (checkLoginStatus()) { log(''); log('Already logged in'); log(''); console.log(JSON.stringify({ status: 'already_logged_in' })); process.exit(0); }
log(''); log('Starting login authorization...'); log('');
